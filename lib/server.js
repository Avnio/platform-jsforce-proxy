...
app.get('/qry/', cors(corsOptions), async function (req, res) {
	...
	let sandbox = req.query.sandbox;
	let sfToken = req?.headers?.authorization ? req.headers.authorization : req.query.token;
	conn = new jsforce.Connection({
...
app.get('/kbsearch/', cors(corsOptions), async function (req, res) {
	...
	let userActivity =  {
		...
		transactionId: qryParams.transactionId
	}
...
app.get('/libconfigdetailfetch/', cors(corsOptions), async function (req, res) {
	let sfToken = req?.headers?.authorization ? req.headers.authorization : req.query.token;
	let qryParams = req.query;
	try {
		var requestOptions = {
		...
		let response;
		response = await fetch(qryParams.instanceURL+"/services/apexrest/avnio/LibraryConfigDetails", requestOptions);
		if(response.status === 200){
			response = await response.text();
			res.send(response);
		}
...
app.get('/', function (req, res) {
	res.send('JSforce AJAX Proxy Server Running!!');
});
